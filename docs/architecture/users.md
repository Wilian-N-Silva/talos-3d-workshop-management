...
password hash, status, fixed Release 1 role, creation/update timestamps, and an
...
Malformed or resource-unsafe hashes fail closed. Migration
`00006_user_roles.sql` adds a checked role value; authorization resolves that
profile to concrete permissions as documented in [Roles and permission
...
instances therefore cannot create multiple initial owners. New bootstrap
identities are persisted with the `owner` role.
...
as the initial-owner marker. When the role migration is applied, that durable
identity becomes `owner`; any other existing identity is assigned
