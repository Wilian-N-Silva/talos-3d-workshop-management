...
func TestCatalogItemRepositoryAgainstPostgreSQL(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = database.ExecContext(ctx, "TRUNCATE TABLE design_version_files, design_versions, catalog_parts, catalog_items")
		_ = database.Close()
	})
	if err := Migrate(ctx, database); err != nil {
		t.Fatalf("Migrate() error = %v", err)
	}
	if _, err := database.ExecContext(ctx, "TRUNCATE TABLE design_version_files, design_versions, catalog_parts, catalog_items"); err != nil {
