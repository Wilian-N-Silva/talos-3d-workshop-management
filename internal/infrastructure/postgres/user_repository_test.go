...
func TestUserRepositoryAgainstPostgreSQL(t *testing.T) {
	...
	if _, err := database.ExecContext(ctx, "TRUNCATE TABLE spool_measurements, material_spools, materials, design_version_files, design_versions, catalog_parts, catalog_items, workshop_settings, files, sessions, bootstrap_state, users"); err != nil {
