...
func TestPrinterRepositoryAgainstPostgreSQL(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = database.ExecContext(ctx, "TRUNCATE TABLE energy_measurements, print_job_material_usage, job_events, print_jobs, printers")
		_ = database.Close()
	})
	if err := Migrate(ctx, database); err != nil {
		t.Fatalf("Migrate()=%v", err)
	}
	if _, err := database.ExecContext(ctx, "TRUNCATE TABLE energy_measurements, print_job_material_usage, job_events, print_jobs, printers"); err != nil {
